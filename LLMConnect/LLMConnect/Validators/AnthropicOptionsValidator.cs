...
internal class AnthropicOptionsValidator : ILLMProviderOptionsValidator
{
    public void Validate(LLMConnectClientOptions options)
    {
        if (string.IsNullOrWhiteSpace(options.ApiKey))
            throw new ArgumentException("API key is required for Anthropic.", nameof(options.ApiKey));
    ...
    private static void ValidateEndpoint(LLMConnectClientOptions options, bool allowEmpty)
    {
        ...
        {
            if (!allowEmpty)
                throw new ArgumentException("Endpoint is required for Anthropic.", nameof(options.Endpoint));
            return;
        }
    ...
    private static void ValidateHttpsEndpoint(LLMConnectClientOptions options)
    {
        ...
        if (uri.Scheme != Uri.UriSchemeHttps && uri.Host != "localhost" && uri.Host != "127.0.0.1")
            throw new ArgumentException("Endpoint must use HTTPS for Anthropic.", nameof(options.Endpoint));
    }
}