...
{
    private const string Resource = "my-resource";
    private const string Deployment = "my-deployment";
    private const string ApiVersion = "2024-02-15-preview";
    private const string ChatPath = $"/openai/deployments/{Deployment}/chat/completions";
    private const string EmbedPath = $"/openai/deployments/{Deployment}/embeddings";

    public AzureOpenAIIntegrationTests()
        : base(ProviderType.AzureOpenAI,
               CreateAzureEndpointOptions(Resource, Deployment, ApiVersion),
               requiresApiKey: true)
    {
    }
    ...
    public async Task ChatAsync_Retry_SucceedsAfter429()
    {
        ...
        StubRetryScenario(ChatPath, fail, success,
            configureRequest: req => req.WithParam("api-version", ApiVersion));
